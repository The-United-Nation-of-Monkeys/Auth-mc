...
from src.notification.forms import register_form, not_register_info_mail_form
...
def send_info_not_register_mail(recipient: str, name: str) -> None:
    with smtplib.SMTP_SSL("smtp.mail.ru", 465) as session:
        session.login(settings.mail.mail, settings.mail.password)
        content = not_register_info_mail_form.format(name=name)
